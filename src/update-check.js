/**
 * ScanVerse — in-app update orchestrator (RPCS3-style flow).
 *
 ...
 *    BrowserWindow (src/update-ui/) so the user sees a ScanVerse-skinned
 ...
 *      (where X is the current ScanVerse install dir, taken from
 ...
 *      uninstall key → launch the new ScanVerse.exe.
 ...
 *   - Silent install fails → see %TEMP%/scanverse-installer.log. The old
...
function registerIpc() {
  ...
  ipcMain.handle('update:download', async () => {
    ...
    const tmpPath = path.join(os.tmpdir(), `ScanVerse-Setup-${latest}.exe`);
  ...
  ipcMain.on('update:apply', () => {
    ...
    // process.execPath = path/to/ScanVerse.exe in the install dir.
