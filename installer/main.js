/**
 * ScanVerse Setup — custom Electron installer.
 *
 ...
 *     └── payload/              ← the actual ScanVerse app bytes
 *         └── (win-unpacked/)
 *
 * On install:
 *   1. User picks an install path (default %LOCALAPPDATA%\Programs\ScanVerse).
 *   2. We copy `payload/` into that path.
 *   3. We register an uninstall key in HKCU so Apps & features lists ScanVerse.
...
function createWindow() {
  win = new BrowserWindow({
    ...
    title: 'Installation de ScanVerse',
    icon: ICON_PATH,
...
ipcMain.handle('installer:pick-folder', async () => {
  ...
  // We always append "ScanVerse" so the user doesn't accidentally install
  // into a populated directory. Skip the append if the leaf already is it.
  let target = r.filePaths[0];
  if (path.basename(target).toLowerCase() !== 'scanverse') {
    target = path.join(target, 'ScanVerse');
  }
  return target;
});
...
ipcMain.handle('installer:install', async (_event, opts = {}) => {
  ...
  try {
    ...
    const exePath = path.join(installPath, 'ScanVerse.exe');
    if (!fs.existsSync(exePath)) {
      return { ok: false, error: `ScanVerse.exe manquant dans ${installPath}` };
    }
    ...
    if (createStartMenu) {
      await createShortcut({
        ...
        location: path.join(
          ...
          'Microsoft', 'Windows', 'Start Menu', 'Programs', 'ScanVerse.lnk',
        ),
      });
    }
    if (createDesktop) {
      await createShortcut({
        target: exePath,
        location: path.join(
          os.homedir(), 'Desktop', 'ScanVerse.lnk',
        ),
      });
    }
...
async function copyDir(src, dst, onProgress) {
  ...
  // `resources/app.asar` (the main ScanVerse app bundled as an asar), and
...
async function writeUninstaller(installPath) {
  ...
  const cmd = `@echo off
chcp 65001 > nul
echo Désinstallation de ScanVerse en cours...
taskkill /IM ScanVerse.exe /F > nul 2>&1
timeout /t 1 /nobreak > nul
reg delete "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\ScanVerse" /f > nul 2>&1
del "%APPDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\ScanVerse.lnk" > nul 2>&1
del "%USERPROFILE%\\Desktop\\ScanVerse.lnk" > nul 2>&1
cd /d "%TEMP%"
rmdir /s /q "${installPath}" > nul 2>&1
echo ScanVerse a été désinstallé.
...
function registerUninstall(installPath, exePath) {
  return new Promise((resolve, reject) => {
    ...
    const key = 'HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\ScanVerse';
    const sets = [
